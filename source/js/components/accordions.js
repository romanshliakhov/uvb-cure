...
window.addEventListener('DOMContentLoaded', () => {
  accParrent &&
    accParrent.forEach(function (accordionParrent) {
      if (accordionParrent) {
        ...
        const closeAccordion = function (accordion, className = 'active') {
          accordion.style.maxWidth = 211 + 'px';

          accordion.querySelector('p').style.maxHeight = 0;
        ...
        const openAccordion = function (accordion, className = 'active') {
          ...
          accordion.querySelector('p').style.maxHeight = accordion.querySelector('p').scrollHeight + 'px';
              ...
              const mobileSettings = () => {
                ...
                if (containerWidth <= breakpointSetting && multipleSetting === 'true') {
                  ...
                  toggleAccordionButton(
                    accordionParrent.querySelector(`[data-id="${openedAccordion.dataset.content}"]`)
                  );
                }
              };
